...
# ---- DR08 / RBox prompt parsing (from executor stdout) ----
# Example:
#   Set DR08 to R ≈ 1000.0 Ω (requested 1000.0 Ω)
#   Digits [1M,100k,10k,1k,100,10,1,0.1]:
#   0 0 0 1 0 0 0 0
mR = re.search(r"Set DR08 to R ≈\s*([0-9.+-eE]+)\s*Ω\s*\(requested\s*([0-9.+-eE]+)\s*Ω\)", chunk)
if mR:
    target = mR.group(1)
    req = mR.group(2)
    self.after(0, lambda t=target, r=req: (self.v_rbox_target.set(f"{t} Ω"), self.v_rbox_requested.set(f"{r} Ω")))

if "Digits [1M,100k,10k,1k,100,10,1,0.1]:" in chunk:
    self._awaiting_rbox_digits = True

if self._awaiting_rbox_digits:
    md = re.search(r"^\s*([0-9])\s+([0-9])\s+([0-9])\s+([0-9])\s+([0-9])\s+([0-9])\s+([0-9])\s+([0-9])\s*$", chunk)
    if md:
        digs = [md.group(i) for i in range(1, 9)]
        def _set_digits(vals=digs):
            for i, v in enumerate(vals):
                self.v_rbox_digits[i].set(v)
        self.after(0, _set_digits)
